...
cbuffer TexInfo : register(b1)
{
	...
	float fA; // specular sharpness
	float fH; // diffuse sharpness
	float fBumpStrength;
	float frTransparency;
	float4 f4ColorSSS;
	float4 f4SSSBRDFParams;
	int aiTexFlag[24]; 
	// 0 = Image,		1 = Glow,			 2 = Specular,		 3 = Bump
	// 4 = ImageBlack,	5 = GlowAsImage,	 6 = SpecularAsImage,7 = HeightAsImage
	// 8 = ImageWhite,	9 = GlowWhite,		10 = SpecularWhite,	11 = Height
	//12 = ShadingOn   13 = GlowAsAmbient	14 = ChromaKeying   15 = Environment
	//16 = POM		   17 = TexBRDF         18 = TexSSS         19 = 
...
cbuffer ViewportArt : register(b2)
{
	...
	bool abArt[16];
	// 0 = Colorize,		1= Halftransparent,		 2 = PopArt,		3 = Outline, 4 = Monocrome,  5 = Own, 6 = Nebel
}
...
Texture2D tex2D[7] : register (t12); // texture
...
{
	if (aiTexFlag[22]>1 || aiTexFlag[23]>1) // Animierte 
	{
		//20 = ixPos       21 = iyPos           22 = ixPics         23 = iyPics 
	...
	float fSpecular = 0;
	float4 f4SpecCol;
	if (aiTexFlag[2]) // TEXTFLAG_SPECULAR
	{
		[unroll]
		for (uint i = 0; i < input.iLightCount; i++)
		{
		...
		if (aiTexFlag[10]) // TEXFLAG_SPECULARWHITE
			f4SpecCol = 2 * (fSpecular * f4Diffuse);
		else if (aiTexFlag[6])// TEXFLAG_SPECULARASIMAGE 
	...
	float4 f4Glow;
	if (aiTexFlag[1]) // TEXFLAG_GLOW
	{
		if (aiTexFlag[13]) // TEXFLAG_GLOWASAMBIENT
			f4Glow = f4ColorAmbient;
		else
		{
			if (aiTexFlag[5]) // TEXFLAG_GLOWASIMAGE
	...
	if (abArt[1]) // Halftransparent
	{
		f4ColorOut *= 0.5;
	}
	if (abArt[3]) // Outline-Shading
	{
		float fEdge = max(dot(normalize(input.f3CamPos - (float3)input.f4VertexPos), input.f3Normal), 0);
	...
	if (abArt[0]) // Style Colorize
	{
		f4ColorOut = mul(f4ColorOut, mColorModification);
	}

	if (abArt[5]) // Own-Shading
	{
